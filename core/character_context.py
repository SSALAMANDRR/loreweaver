...
def _candidate_sidecars(pack: Any, data_root: Path | None = None) -> list[Path]:
    ...
    if not system:
        raise CharacterContextError("pack has no system id")
    if any(part in system for part in ("/", "\\", "..")):
        raise CharacterContextError("pack system id is not safe for a character-context path")
...
def load_character_context_spec(pack: Any, *, data_root: Path | None = None) -> Mapping[str, Any] | None:
    ...
    try:
        ...
        raise CharacterContextError(f"could not load character-context sidecar {path.name!r}: {exc}") from exc
    if not isinstance(raw, Mapping):
        raise CharacterContextError("character-context sidecar root must be a mapping")
    if int(raw.get("version", 1)) != 1:
        raise CharacterContextError("unsupported character-context sidecar version")
    unknown = set(raw) - {"version", "optional", "fields"}
    if unknown:
        raise CharacterContextError(f"character-context sidecar has unknown root keys {sorted(unknown)}")
    optional = raw.get("optional", True)
    if not isinstance(optional, bool):
        raise CharacterContextError("character-context optional must be boolean")
    fields = raw.get("fields") or {}
    if not isinstance(fields, Mapping) or not fields:
        raise CharacterContextError("character-context fields must be a non-empty mapping")
    ...
    for field_id_raw, field_raw in fields.items():
        ...
        if not field_id or field_id in seen or not isinstance(field_raw, Mapping):
            raise CharacterContextError("character-context contains an invalid field")
        seen.add(field_id)
        extra = set(field_raw) - {"kind", "required", "display", "options", "placeholder"}
        if extra:
            raise CharacterContextError(
                f"character-context field {field_id!r} has unknown keys {sorted(extra)}"
            )
        kind = str(field_raw.get("kind") or "text").strip().casefold()
        if kind not in _ALLOWED_FIELD_KINDS:
            raise CharacterContextError(
                f"character-context field {field_id!r}.kind must be one of {sorted(_ALLOWED_FIELD_KINDS)}"
            )
        required = field_raw.get("required", False)
        if not isinstance(required, bool):
            raise CharacterContextError(f"character-context field {field_id!r}.required must be boolean")
        options = field_raw.get("options") or {}
        if kind == "choice":
            if not isinstance(options, Mapping) or not options:
                raise CharacterContextError(f"character-context choice {field_id!r} requires options")
            for option_id_raw, option_raw in options.items():
                ...
                if not option_id or not isinstance(option_raw, Mapping):
                    raise CharacterContextError(f"character-context choice {field_id!r} has invalid option")
                option_extra = set(option_raw) - {"display"}
                if option_extra:
                    raise CharacterContextError(
                        f"character-context choice {field_id!r}.{option_id!r} has unknown keys {sorted(option_extra)}"
                    )
        elif options:
            raise CharacterContextError(f"character-context field {field_id!r} may use options only when kind=choice")
    return raw
...
def _state(character: Any) -> Mapping[str, Any] | None:
    ...
    if not isinstance(secondary, dict):
        raise CharacterContextError("character secondary attribute storage must be a mapping")
    raw = secondary.get(_STATE_KEY)
    if raw is None:
        return None
    if not isinstance(raw, Mapping):
        raise CharacterContextError("character-context state must be a mapping")
    if raw.get("version") != 1:
        raise CharacterContextError("unsupported character-context state version")
    values = raw.get("values") or {}
    if not isinstance(values, Mapping):
        raise CharacterContextError("character-context values must be a mapping")
    return raw
...
def set_character_context(
    ...
    *,
    ...
    spec = load_character_context_spec(pack)
    if spec is None:
        raise CharacterContextError("rulepack has no character-context sidecar")
    secondary = getattr(character, "secondary_attributes", None)
    if not isinstance(secondary, dict):
        raise CharacterContextError("character secondary attribute storage must be a mapping")
    if skip:
        if not bool(spec.get("optional", True)):
            raise CharacterContextError("character-context form is not optional")
    ...
    if unknown:
        raise CharacterContextError(f"unknown character-context fields: {sorted(unknown)}")
    ...
    for field_id_raw, field_raw_any in fields.items():
        ...
        if not isinstance(value_raw, str):
            raise CharacterContextError(f"character-context field {field_id!r} must be text")
        value = value_raw.strip()
        if len(value) > _MAX_TEXT:
            raise CharacterContextError(f"character-context field {field_id!r} is too long")
        if value:
            any_value = True
        kind = str(field_raw_any.get("kind") or "text").casefold()
        if kind == "choice" and value:
            ...
            if value not in {str(option_id) for option_id in options}:
                raise CharacterContextError(f"invalid character-context choice for {field_id!r}")
    ...
    if missing:
        raise CharacterContextError(f"required character-context fields are missing: {', '.join(missing)}")
